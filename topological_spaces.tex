...
\begin{proof}
  \pf
  \step{<1>1}{\assume{$\{ \supp f_\alpha \}_{\alpha \in J}$ is locally finite.}}
  \step{<1>2}{\pflet{$x \in X$}}
  \step{<1>3}{\pick\ an open neighbourhood $U$ of $x$ that intersects only $\supp f_\alpha$ for only finitely many $\alpha$, say $\alpha_1$, \ldots, $\alpha_n$}
  \begin{proof}
    \pf\ \stepref{<1>1}, \stepref{<1>2}
  \end{proof}
  \step{<1>4}{For all $\alpha \in J$, if $f_\alpha(x) = 0$ then $\alpha$ is one of $\alpha_1$, \ldots, $\alpha_n$.}
  \begin{proof}
    \pf\ \stepref{<1>3}, Proposition \ref{prop:topology:closure:A_sub_Abar}.
  \end{proof}
  \qed
\end{proof}
